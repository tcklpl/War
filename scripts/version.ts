...
try {
	[major, minor, patch] = [major, minor, patch].map(v => Number.parseInt(v));
	if (Number.isNaN(major) || Number.isNaN(minor) || Number.isNaN(patch)) throw Error();
} catch {
	exitInvalidVersion();
}
...
const packages = ['./game/package.json', './server/package.json', './protocol/package.json'];
...
// biome-ignore lint/complexity/noUselessEmptyExport: needed
export {};
