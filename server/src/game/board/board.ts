import type { GraphNodeTerritoryPacket, GraphTerritoryPacket } from ':protocol';
...
export class Board {
	...
	private registerGraphEdges() {
        ...
        */
		...
		this.territories.mackenzie.addAdjacentNode(
			'land',
			this.territories.alaska,
			this.territories.vancouver,
			this.territories.ottawa,
		);
		this.territories.mackenzie.addAdjacentNode('sea', this.territories.greenland);

		this.territories.vancouver.addAdjacentNode(
			'land',
			this.territories.alaska,
			this.territories.mackenzie,
			this.territories.ottawa,
			this.territories.california,
		);

		this.territories.ottawa.addAdjacentNode(
			'land',
			this.territories.mackenzie,
			this.territories.mackenzie,
			this.territories.vancouver,
			this.territories.california,
			this.territories.new_york,
			this.territories.labrador,
		);

		this.territories.california.addAdjacentNode(
			'land',
			this.territories.vancouver,
			this.territories.ottawa,
			this.territories.new_york,
			this.territories.mexico,
		);

		this.territories.new_york.addAdjacentNode(
			'land',
			this.territories.labrador,
			this.territories.ottawa,
			this.territories.california,
		...
		this.territories.mexico.addAdjacentNode(
			'land',
			this.territories.california,
			this.territories.new_york,
			this.territories.venezuela,
		);
		this.territories.mexico.addAdjacentNode('sea', this.territories.cuba);

		this.territories.cuba.addAdjacentNode(
			'sea',
			this.territories.new_york,
			this.territories.mexico,
			this.territories.venezuela,
		...
		this.territories.greenland.addAdjacentNode(
			'sea',
			this.territories.mackenzie,
			this.territories.labrador,
			this.territories.iceland,
		);

		/*
            South America
        */
		this.territories.venezuela.addAdjacentNode(
			'land',
			this.territories.mexico,
			this.territories.brazil,
			this.territories.chile,
		);
		this.territories.venezuela.addAdjacentNode('sea', this.territories.cuba);

		this.territories.chile.addAdjacentNode(
			'land',
			this.territories.venezuela,
			this.territories.brazil,
			this.territories.argentina,
		);

		this.territories.brazil.addAdjacentNode(
			'land',
			this.territories.venezuela,
			this.territories.chile,
			this.territories.argentina,
        ...
        */
		...
		this.territories.england.addAdjacentNode(
			'sea',
			this.territories.iceland,
			this.territories.sweden,
			this.territories.germany,
			this.territories.france,
		);
		...
		this.territories.poland.addAdjacentNode(
			'land',
			this.territories.france,
			this.territories.germany,
			this.territories.moscow,
			this.territories.middle_east,
		);
		this.territories.poland.addAdjacentNode('sea', this.territories.sweden);

		this.territories.moscow.addAdjacentNode(
			'land',
			this.territories.sweden,
			this.territories.poland,
			this.territories.middle_east,
			this.territories.aral,
			this.territories.ornsk,
		);

		this.territories.sweden.addAdjacentNode('land', this.territories.moscow);
		this.territories.sweden.addAdjacentNode(
			'sea',
			this.territories.england,
			this.territories.germany,
			this.territories.poland,
		);
		...
		/*
            Africa
        */
		this.territories.algeria.addAdjacentNode(
			'land',
		...
		/*
            Oceania
        */
		this.territories.australia.addAdjacentNode(
			'sea',
			this.territories.sumatra,
			this.territories.borneo,
			this.territories.new_guinea,
		);

		this.territories.sumatra.addAdjacentNode(
			'sea',
			this.territories.australia,
			this.territories.borneo,
			this.territories.india,
			this.territories.vietnam,
		);

		this.territories.borneo.addAdjacentNode(
			'sea',
			this.territories.australia,
			this.territories.sumatra,
			this.territories.new_guinea,
		...
		/*
            Asia
        */
		this.territories.middle_east.addAdjacentNode(
			'land',
			this.territories.egypt,
			this.territories.poland,
			this.territories.moscow,
		...
		this.territories.ornsk.addAdjacentNode(
			'land',
			this.territories.dudinka,
			this.territories.mongolia,
			this.territories.china,
			this.territories.aral,
			this.territories.moscow,
		);

		this.territories.aral.addAdjacentNode(
			'land',
			this.territories.ornsk,
			this.territories.moscow,
			this.territories.middle_east,
		...
		this.territories.india.addAdjacentNode(
			'land',
			this.territories.middle_east,
			this.territories.aral,
			this.territories.china,
		...
		this.territories.dudinka.addAdjacentNode(
			'land',
			this.territories.ornsk,
			this.territories.mongolia,
			this.territories.tchita,
			this.territories.siberia,
		);

		this.territories.mongolia.addAdjacentNode(
			'land',
			this.territories.dudinka,
			this.territories.ornsk,
			this.territories.china,
		...
		this.territories.china.addAdjacentNode(
			'land',
			this.territories.mongolia,
			this.territories.ornsk,
			this.territories.aral,
			this.territories.india,
			this.territories.vietnam,
			this.territories.vladvostok,
			this.territories.tchita,
		);
		this.territories.china.addAdjacentNode('sea', this.territories.japan);

		this.territories.tchita.addAdjacentNode(
			'land',
			this.territories.siberia,
			this.territories.dudinka,
			this.territories.mongolia,
		...
		this.territories.siberia.addAdjacentNode(
			'land',
			this.territories.dudinka,
			this.territories.tchita,
			this.territories.vladvostok,
		...
		this.territories.vladvostok.addAdjacentNode(
			'land',
			this.territories.siberia,
			this.territories.tchita,
			this.territories.china,
		...
		for (const n of Array.from(this.graph.nodes.values())) {
			...
			for (const an of Array.from(n.landAdjacentNodes)) {
				if (!an.landAdjacentNodes.has(n)) {
					this._log.error(
						`While validating Territory Graph: Node "${an.data.code}" is missing bidirectional connection to node "${n.data.code}" via "land"`,
					);
				}
			}
			// by sea
			for (const an of Array.from(n.seaAdjacentNodes)) {
				if (!an.seaAdjacentNodes.has(n)) {
					this._log.error(
						`While validating Territory Graph: Node "${an.data.code}" is missing bidirectional connection to node "${n.data.code}" via "sea"`,
					);
				}
			}
		}
	...
	get asGraphTerritoryPacket(): GraphTerritoryPacket {
		return {
			territories: Array.from(this.graph.nodes.values()).map(
				n =>
					<GraphNodeTerritoryPacket>{
						code: n.data.code,
