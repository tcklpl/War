...
export class Logger {
	...
	static saveCrashLog(error: Error) {
		...
		const fileName = `${Logger._crashLogFolder}/${formatDate(moment, Logger._crashLogFileNameFormat)}`;
		let logContent = Logger._logBuffer
			.join('\n')
	...
	private static parseLogLevelFromString(s: string) {
		switch (s.trim().toLowerCase()) {
			...
			default:
				Logger._privateLogger.warn(`Invalid log level '${s}', defaulting to INFO`);
	...
	static configureLogger(cfg: LoggerConfig) {
		Logger._logLevel = Logger.parseLogLevelFromString(cfg.log_level);
		Logger._logTimeFormat = cfg.log_time_format;
		Logger._logBufferMaxSize = cfg.log_max_buffer_lines;
		Logger._crashLogFolder = `${process.cwd()}/${cfg.log_crashlog_folder}`;
		Logger._crashLogFileNameFormat = cfg.log_crashlog_file_name;
		Logger._logBufferIgnoreLogLevel = cfg.log_crashlog_ignore_log_level;
	}
